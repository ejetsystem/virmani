...
if (isset($_POST["btn_admin"])) {
    ...
    if (mysqli_connect_errno()) {
        ...
        mysqli_query($connection, "ALTER TABLE `settings` ADD `sid` VARCHAR(255) NULL DEFAULT '2020-02-02' AFTER `lang`;");

        mysqli_query($connection, "UPDATE `package` SET `name` = 'Basic' WHERE `package`.`id` = 1;");
        ...
        mysqli_query($connection, "ALTER TABLE `users` ADD `city` VARCHAR(255) NULL DEFAULT NULL AFTER `country`;");

        mysqli_query($connection, "ALTER TABLE `users` ADD `qr_code` VARCHAR(255) NULL DEFAULT NULL AFTER `trial_expire`;");

        mysqli_query($connection, "INSERT INTO `lang_values` (`type`, `label`, `keyword`, `english`) VALUES
        ('user', 'Basic', 'basic', 'Basic'),
        ('user', 'Standard', 'standard', 'Standard'),
        ...
        ('user', 'Your payment has been completed Successfully', 'your-payment-has-been-completed-successfully', 'Your payment has been completed Successfully'),
        ('user', 'Your payment has been failed', 'your-payment-has-been-failed', 'Your payment has been failed'),
        ('user', 'City', 'city', 'City'),
        ('user', 'QR Code', 'qr-code', 'QR Code'),
        ('user', 'Share QR Code', 'share-qr-code', 'Share QR Code'),
        ('user', 'Download', 'download', 'Download'),
        ('user', 'Please add SSL Certificate (https) on your url to enable Zoom meeting', 'zoom-integration-alert', 'Please add SSL Certificate (https) on your url to enable Zoom meeting'),
        ('user', 'Zoom Integration doc', 'zoom-integration-doc', 'Zoom Integration doc'),
        ('user', 'Zoom API Key', 'zoom-api-key', 'Zoom API Key'),
        ('user', 'Zoom Secret Key', 'zoom-secret-key', 'Zoom Secret Key');");
