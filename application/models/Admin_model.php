...
// Percentase Resize
            if ($limit_use > $limit_big || $limit_use > $limit_thumb || $limit_use > $limit_medium || $limit_use > $limit_doc || $limit_use > $limit_patients) {
                ...
                $percent_medium  = $limit_medium/$limit_use ;
                $percent_thumb  = $limit_thumb/$limit_use ;
                $percent_doc  = $limit_doc/$limit_use ;
...
// Do Resizing
            ...
            $data_image = array(
                ...
                'doc' => 'uploads/doctor-thumbnail/'.$doc_thumb,
                'patients' => 'uploads/patientses/'.$patientses_thumb
            );
...
//check unique language name
    ...
    function select_chamber_by_user($table){
        $this->db->select();
        $this->db->from($table);
        if ($this->session->userdata('role') == 'user') {
            $user_id = $this->session->userdata('id');
        ...
        $this->db->where_in('user_id',array(1,$user_id));
        $this->db->order_by('id','DESC');
        $query = $this->db->get();
        $query = $query->result();  
        return $query;
    ...
    function insert_doctors_vaccination($d,$table,$param=""){
        ...
        for ($i=0; $i <count($d['vaccination_type']) ; $i++) { 
            $remarks = (!empty($d['remarks'][$i])) ? $d['remarks'][$i] : '';
            $medical_history = (!empty($d['medical_history'][$i])) ? $d['medical_history'][$i] : '';
            $this->db->insert($table,array('doctor_id'=>$d['doctor_id'],'vaccination_type'=>$d['vaccination_type'][$i],'vaccination_date'=>$d['vaccination_date'][$i],'reminder_date_for_next'=>$d['reminder_date_for_next'][$i],'remarks'=>$remarks,'medical_history'=>$medical_history,'updated_at'=>date("Y-m-d h:i:s")));
        }
        return true;
    }

    function insert_doctors_banks($d,$table,$param=""){
        ...
        for ($i=0; $i <count($d['bank_name']) ; $i++) { 
            $remarks = (isset($d['remarks'][$i])) ? $d['remarks'][$i] : '';
            $this->db->insert($table,array('doctor_id'=>$d['doctor_id'],'bank_name'=>$d['bank_name'][$i],'bank_account_number'=>$d['bank_account_number'][$i],'ifsc_code'=>$d['ifsc_code'][$i],'remarks'=>$remarks,'updated_at'=>date("Y-m-d h:i:s")));
        }
    }

    function insert_doctors_insurance($d,$table,$param=""){
        ...
        for ($i=0; $i <count($d['insurance']) ; $i++) { 
            ...
            $remarks = (isset($d['remarks'][$i])) ? $d['remarks'][$i] : '';
            $this->db->insert($table,array('doctor_id'=>$d['doctor_id'],'insurance'=>$d['insurance'][$i],'insurance_date'=>$d['insurance_date'][$i],'renewal_date'=>$d['renewal_date'][$i],'amount_insured'=>$d['amount_insured'][$i],'amount_paid'=>$d['amount_paid'][$i],'allow_notifications'=>$notifications,'remarks'=>$remarks,'updated_at'=>date("Y-m-d h:i:s")));
        }
    }

    function select_doctor_detail($d,$table){
    ...
    function select_vaccination($id,$table){
        $this->db->select('id as vaccination_id,vaccination_type,vaccination_date,reminder_date_for_next,remarks as vaccination_remarks,medical_history');
        $this->db->from($table);
    ...
    function select_bank($id,$table){
        $this->db->select('id as bank_id,bank_name,bank_account_number,ifsc_code,remarks as bank_remarks');
        $this->db->from($table);
        $this->db->where('doctor_id',$id);
        $query = $this->db->get();
        $query = $query->result();  
    ...
    function select_insurance($id,$table){
        $this->db->select('id as insurance_id,insurance,insurance_date,renewal_date,amount_insured,amount_paid,allow_notifications,remarks as insurance_remarks');
        $this->db->from($table);
        $this->db->where('doctor_id',$id);
        $query = $this->db->get();
        $query = $query->result();  
    ...
    public function update_multiple_patient(){

    }

    function select_patients($array,$table){
        $this->db->select('*');
        $this->db->from($table);
        $this->db->where($array);
        $query = $this->db->get();
    ...
    function select_all_patients($table,$is_deleted){
        ...
        $this->db->from($table);
        $this->db->where('chamber_id', $this->chamber->uid);
        $this->db->where('is_delete', $is_deleted);
        $this->db->order_by('id','DESC');
        $query = $this->db->get();
        $query = $query->result();  
        return $query;
