...
class Live_consults extends Home_Controller {
    ...
    public function index()
    {
        ...
        $data['consult'] = FALSE;
        $data['appointments'] = $this->admin_model->get_appointmentsByID(user()->id);
        $data['patients'] = $this->admin_model->select_by_chamber('patientses');
        $data['main_content'] = $this->load->view('doctor/live_consults',$data,TRUE);
    ...
    {  
        if ($_POST) {
            ...
            redirect(base_url('doctor/live_consults'));
        }
        $data = array();
    ...
    {  
        ...
        $data['main_content'] = $this->load->view('doctor/evisit_settings',$data,TRUE);
        $data['doctors'] = $this->admin_model->select_all_doctors();
        $data['patientses'] = $this->admin_model->select_by_chamber('patientses');
        $this->load->view('doctor/index',$data);
    }
    ...
    public function evisit_settings() 
    {
        ...
        redirect(base_url('doctor/live_consults/settings'));
    }
